...
struct Node
{
  ...
  Node(const string& value, Node* const parent) : value(value) { parent->next = this; }
};
...
struct ListIterator : boost::iterator_facade<ListIterator, Node, boost::forward_traversal_tag>
{
 ...
 private:
  ...
  bool equal(const ListIterator& other) const { return other.current == current; };
...
int main()
{
  ...
  for_each(ListIterator{&alpha}, ListIterator{}, [](const Node& n) { cout << n.value << endl; });
